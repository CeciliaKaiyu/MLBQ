...
def MLBQ(L, N, solver, sampler, hyper, kernel, KM, CalIE=False, IE=None, CalMLMC=True, sum=True, nuggetL=None, method="LBFGS", epochL=None, bs=None, maxBatch_size=5, lrL=None, betasL=None, epsL=None, weight_decayL=None, max_iterL=None):
    '''
    ...
    :param bs: a vector of specified batch size for each level , if it is none, set bs to be the min of (sample_size, maxBatch_size)
    :param maxBatch_size: maximum batch size (default:5)
   ...
   '''
    ...
    for l in range(L):
        ...
        if CalIE:
            E, V = BQ(X=X, Y=Y, Gram=Gram, hyper=hyper, kernel=kernel, KM=KM, CalIE=CalIE, IE=IE,  nugget=nuggetL[l], method=method, epoch=epochL[l], batch_size=int(bs[l]),  lr=lrL[l], betas=betasL[l], eps=epsL[l], weight_decay=weight_decayL[l], max_iter = max_iterL[l])
            Est[l,0] = float(E)
            Est[l,1] = float(V)
            if CalMLMC:
            ...
            E = BQ(X=X, Y=Y, Gram=Gram, hyper=hyper, kernel=kernel, KM=KM, CalIE=CalIE, IE=IE,  nugget=nuggetL[l], method=method, epoch=epochL[l], batch_size=int(bs[l]),  lr=lrL[l], betas=betasL[l], eps=epsL[l], weight_decay=weight_decayL[l], max_iter = max_iterL[l])
            Est[l,0] = float(E)
            if CalMLMC:
                # obtain MC estimate at level l
                Est[l, 1] = float(torch.mean(Y))
                Est[l, 2] = float(torch.var(Y) / N[l])
